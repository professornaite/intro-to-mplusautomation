...
**What can we do with Mplus output in R?**

What makes `MplusAutomation` powerful is its ability to read output and create tables and figures. Let's look at the `basic.out` example. First, read in the output using the `readModels` function:

```{r}
out <- readModels(here("part2_rstudio", "basic.out"))
out
```

When looking at the `out` object on its own, we see the fit statistics and parameters of the model. What if we want to look at the all the summary statistics?

```{r}
out$sampstat
```

Specifically, lets look at the correlation table:

```{r}
out$sampstat$correlations
```
